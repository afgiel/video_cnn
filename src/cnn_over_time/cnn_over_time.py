...
def get_data():
	...
	print '\tnumpying' 
	X_train = np.array(X_train).reshape((train_size, 10, 64, 64))
 	y_train = np.array(y_train)
 	X_valid = np.array(X_valid).reshape((split_size, 10, 64, 64))
 	y_valid = np.array(y_valid)
 	X_test = np.array(X_test).reshape((len(X_test), 10, 64, 64))
...
def build_model(input_dim, output_dim, 
    ...
    l_in = lasagne.layers.InputLayer(
          shape=(batch_size, input_dim[0], input_dim[1], input_dim[2]),
          )
